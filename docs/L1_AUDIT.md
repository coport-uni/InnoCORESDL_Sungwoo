...
## A8 — Are Cell D's not-yet-L1 devices' drivers available?

Verdict: **gap** (no `cell5` class), but **every driver Cell D needs
exists and its API was read** — the composition is buildable today.

Cell D = 4 devices (user-confirmed 2026-07-27). Per-device readiness:
...
| Tapo plug (IR lamp power) | `external/SmartPlugController` (python-kasa) | `SmartPlugController.from_files()`, `resolve_targets(name)`, `switch_many(entries, turn_on=…)` (**async** — the cell must wrap it, as `demo_scenario/devices.py` did with `asyncio.run`), status/energy read | yes, once configured |
...
- **Cell D's `stop()` is a real stop**, unlike cell4's (GAP-1): it hard-stops
...
| GAP-2 | ~~Cell D (`cell5`) does not exist~~ — **implemented 2026-07-27** | — | `cell/pump_z_thermal_cell.py` + the `zstage`/`hotplate`/`lamp` routes + `server/nuc2/cell5.toml.example`; `[cells.cell5]` is now live in the orchestrator config example. **Remaining: bench bring-up** (S5–S7 below) and the TBD identifiers in GAP-5/6 |
...
| S5 | Z stage (Cell D, 1 × MKS motor) | `smoke_l1.py --suite zstage`: home, ~10 mm out and back, then `POST /v1/stop` mid-move | round trip completes; no X target in the request model; the stop actually halts it | `pending` |
| S6 | IR lamp via Tapo plug (Cell D) | `--suite lamp`: read state, on, off | lamp visibly lights and goes out; the plug's `is_on` agrees. A 409 means `secure.env` is not filled in yet | `pending` |
| S7 | Hotplate (Cell D) | `--suite hotplate`: read, set 30 °C, heater on, hold (operator-timed), heater off | setpoint accepted, temperature trends up, off restores; a setpoint above `max_celsius` is rejected with 400 before the device is touched | `pending`. Never leave the bench while it heats |
| S10 | Cell D end-to-end (L2) | `python -m orchestrator run scenarios/demo_cell_d_warmup.yaml --step-mode` | all 14 steps pass; the run leaves heater off, lamp off, Z home | `pending` |
